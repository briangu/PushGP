...
public abstract class GA implements Serializable
{
  ...
  /**
   * Factor method for creating a GA object, with the GA class specified by the problem-class parameter.
   */
      ...
      throws Exception
  {
    ...
    if (!(gaObject instanceof GA)) throw (new Exception("problem-class must inherit from class GA"));
  ...
  protected String GetParam(String inName)
      throws Exception
  {
   ...
   *
   * @param inName     the name of the parameter.
   * @param inOptional whether the parameter is optional. If a parameter is not optional, this method will throw an
      ...
      throws Exception
  {
    ...
    if (value == null && !inOptional) throw new Exception("Could not locate required parameter \"" + inName + "\"");
  ...
  protected float GetFloatParam(String inName)
      throws Exception
  {
  ...
  /**
   * Sets up the GA object with the previously set parameters. This method is typically overridden to read in custom
   * parameters associated with custom subclasses. Subclasses must always call the base class implementation first to
   * ensure that all base parameters are setup.
   */
      ...
      throws Exception
  {
    ...
    String defaultIndividualClass = "org.ops5.push.engine.PushGPIndividual";
    ...
    if (_outputfile != null) _outputStream = new FileOutputStream(new File(_outputfile));
  }

  /**
   * Sets the population size and resets the GA generation count, as well as initializing the population with random
   * individuals.
   *
   * @param inSize the size of the new GA population.
   */

  protected void ResizeAndInitialize(int inSize)
      throws Exception
  {
  ...
  /**
   * Run the main GP run loop until the generation limit it met, or until the provided number of generations has
   * elapsed.
   *
   * @param inGenerations The maximum number of generations to run during this call. This is distinct from the hard
   *                      generation limit which determines when the GA is actually complete.
   *
   * @return true if the the execution of the GA is complete.
   */

  public boolean Run(int inGenerations)
      throws Exception
  {
  ...
  /**
   * Determine whether the GA should terminate. This method may be overridden by subclasses to customize GA behavior.
   */
  ...
  /**
   * Determine whether the GA has succeeded. This method may be overridden by subclasses to customize GA behavior.
   */
  ...
  /**
   * Evaluates the current population and updates their fitness values. This method may be overridden by subclasses to
  ...
  /**
   * Reproduces the current population into the next population slot. This method may be overridden by subclasses to
   * customize GA behavior.
   */
  ...
  /**
   * Prints out population report statistics. This method may be overridden by subclasses to customize GA behavior.
   */
   ...
   *
   * @param inIndex   The index which is to be replaced by the current reproduction event (used only if trivial-geography
   *                  is enabled).
  ...
  protected float AbsoluteAverageOfErrors(ArrayList<Float> inArray)
  {
    ...
    if (Float.isInfinite(total)) return Float.MAX_VALUE;
  ...
  /**
   * Called at the beginning of each generation. This method may be overridden by subclasses to customize GA behavior.
   *
   * @throws Exception
   */
  ...
  /**
   * Called at the end of each generation. This method may be overridden by subclasses to customize GA behavior.
   */
