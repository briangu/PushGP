...
abstract public class PushGP extends GA
{
      ...
      throws Exception
  {
    ...
    String defaultInterpreterClass = "org.ops5.push.core.Interpreter";
    String defaultInputPusherClass = "org.ops5.push.core.InputPusher";
    ...
    {
      if (!_nodeSelectionMode.equals("unbiased") && !_nodeSelectionMode.equals("leaf-probability") && !_nodeSelectionMode.equals(
          "size-tournament"))
      {
    ...
    if (!(iObject instanceof InputPusher)) throw new Exception("inputpusher-class must inherit from class InputPusher");
    ...
    if (framemode != null && framemode.equals("pushstacks")) _interpreter.SetUseFrames(true);
  ...
  protected void EvaluateIndividual(GAIndividual inIndividual, boolean duringSimplify)
  {
    ...
    if (!duringSimplify) _averageSize += ((PushGPIndividual) inIndividual).Program.programsize();
  ...
  protected String Report()
  {
    ...
    if (Double.isInfinite(_populationMeanFitness)) _populationMeanFitness = Double.MAX_VALUE;
    ...
    PushGPIndividual simplified = Autosimplify((PushGPIndividual) _populations[_currentPopulation][_bestIndividual],
  ...
  protected void Reproduce()
  {
    ...
    for (int n = 0; n < _populations[_currentPopulation].length; n++)
    {
      ...
      else if (method < _crossoverPercent + _mutationPercent + _simplificationPercent)
      {
  ...
  /**
   * Selects a node to use during crossover or mutation. The selection mechanism depends on the global parameter
   * _nodeSelectionMode.
   *
