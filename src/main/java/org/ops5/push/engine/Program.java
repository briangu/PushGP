...
public class Program extends ObjectStack implements Serializable
{
  ...
  public int programsize()
  {
    ...
    {
      Object o = _stack[n];
      if (o instanceof Program) size += ((Program) o).programsize();
    }
  ...
  public int SubtreeSize(int inIndex)
  {
    ...
    if (sub instanceof Program) return ((Program) sub).programsize();
    ...
    else
    {
      ...
      for (int n = 0; n < _size; n++)
      {
        ...
        if (o instanceof Program)
        {
          ...
          if (inIndex - startIndex < length) return sub.ReplaceSubtree(inIndex - startIndex, inReplacement);
  ...
  public String toString()
  {
    ...
    for (int n = 0; n < _size; n++)
    {
      if (result.charAt(result.length() - 1) == '(') result += _stack[n];
  ...
  /**
   * Creates a copy of an object suitable for adding to a Push Program. Java's clone() is unfortunately useless for this
   * task.
   */
  ...
  private Object cloneforprogram(Object inObject)
  {
    ...
    if (inObject instanceof String) return new String((String) inObject);

    if (inObject instanceof Integer) return new Integer((Integer) inObject);

    if (inObject instanceof Float) return new Float((Float) inObject);

    if (inObject instanceof Program) return new Program((Program) inObject);

    if (inObject instanceof Instruction) return inObject; // no need to copy; instructions are singletons
