...
public class FloatRegFitPrediction extends PredictionGA
{
   ...
   * is inOutput. The fitness of the predictor is the absolute error between the prediction and the trainer's actual
   * fitness.
   *
    ...
    for (int n = 0; n < _populations[_currentPopulation].length; n++)
    {
      ...
      for (int k = 0; k < n; k++)
      {
        if (((FloatRegFitPredictionIndividual) next).equalPredictors(_populations[nextPopulation][k]))
        {
          ...
          ((FloatRegFitPredictionIndividual) next).SetSampleIndex(index, _RNG.nextInt(_solutionGA._testCases.size()));
        }
      }
  ...
  /**
   * Mutates an individual by choosing an index at random and randomizing its training point among possible individuals.
   */
  @Override
  ...
  protected GAIndividual ReproduceByCrossover(int inIndex)
  {
    ...
    // gene from b.
    int crossoverPoint = _RNG.nextInt(FloatRegFitPredictionIndividual._sampleSize - 1) + 1;
