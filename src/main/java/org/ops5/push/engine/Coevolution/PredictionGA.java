...
package org.ops5.push.engine.Coevolution;
...
import org.ops5.push.engine.GAIndividual;
import org.ops5.push.engine.PushGP;
import org.ops5.push.engine.PushGPIndividual;


/**
 * An abstract class for a population of co-evolving predictors of fitness, rank, or something similar. In this class,
 * "fitness" or "predicted fitness" of a solution individual may be referring to the actual fitness of the individual,
 * or it may be referring to something similar, such as the individual's rank.
 */
public abstract class PredictionGA extends GA
{
  ...
  /**
   * Customizes GA.GAWithParameters to allow the inclusion of the solution GA, which is required for the initialization
   * of the prediction GA.
   *
   ...
   * @throws Exception
   */
  public static PredictionGA PredictionGAWithParameters(PushGP inSolutionGA, HashMap<String, String> inParams)
      throws Exception
  {
  ...
  /**
   * Chooses a new trainer from the solution population to add to the trainer population. The solution individual is
   * chosen with the highest variance of the predictions from the current predictor population.
   */
  ...
  /**
   * Calculates and sets the exact fitness from any individual of the _solutionGA population. This includes trainers.
   *
   * @param inIndividual
   */
  ...
  /**
   * This must be private, since there must be a _solutionGA set before this method is invoked. Use SetGAandTrainers()
   * instead.
   */
  ...
  /**
   * Evaluates a PredictionGAIndividual's fitness, based on the difference between the predictions of the fitnesses of
   * the trainers and the actual fitnesses of the trainers.
   */
  @Override
  protected abstract void EvaluateIndividual(GAIndividual inIndividual);

  /**
   * Determines the predictor's fitness on a trainer, where the trainer is the inInput, and the trainer's actual fitness
   * (or rank, whatever is to be predicted) is inOutput.
   *
  ...
  /**
   * Actual fitnesses of trainers will always be stored as part of the PushGPIndividual object. Some predictor types,
   * such as rank predictors, will also need a separate storage of data, such as a method of storing the ranking of the
   * predictors. Others, such as fitness predictors, may just need the fitness information directly from the trainers.
   * This function may be used to make sure fitnesses or ranks are updated, i.e. to recalculate rank order with the
   * addition of a new trainer.
   */
