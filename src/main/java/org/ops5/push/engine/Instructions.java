...
/**
 * Abstract instruction class for instructions which operate on any of the built-in stacks.
 */
...
/**
 * Abstract instruction class for instructions which operate on one of the standard ObjectStacks (code & exec).
 */
...
class IntegerAdd extends BinaryIntegerInstruction
{
  ...
  {
    // Test for overflow
    if ((Math.abs(inA) > Integer.MAX_VALUE / 10) || (Math.abs(inB) > Integer.MAX_VALUE / 10))
    {
      long lA = (long) inA;
      long lB = (long) inB;
      if (lA + lB != inA + inB)
...
class IntegerMul extends BinaryIntegerInstruction
{
  ...
  {
    // Test for overflow
    if ((Math.abs(inA) > Math.sqrt(Integer.MAX_VALUE - 1)) || (Math.abs(inB) > Math.sqrt(Integer.MAX_VALUE - 1)))
    {
      long lA = (long) inA;
      long lB = (long) inB;
      if (lA * lB != inA * inB)
...
abstract class UnaryIntInstruction extends Instruction
{
  ...
  public void Execute(Interpreter inI)
  {
    ...
    if (stack.size() > 0) stack.push(UnaryOperator(stack.pop()));
  }
}
...
class IntegerNeg extends UnaryIntInstruction
{
  ...
  {
    // Test for overflow
    if (inValue == Integer.MIN_VALUE) return Integer.MAX_VALUE;
...
class IntegerRand extends Instruction
{
  ...
  public void Execute(Interpreter inI)
  {
    ...
    int randInt = (_RNG.nextInt(range) * inI._randomIntResolution) + inI._minRandomInt;
...
class FloatRand extends Instruction
{
  ...
  public void Execute(Interpreter inI)
  {
    ...
    float randFloat = (_RNG.nextFloat() * range * inI._randomFloatResolution) + inI._minRandomFloat;
...
class BoolNot extends Instruction
{
  ...
  public void Execute(Interpreter inI)
  {
    if (inI.boolStack().size() > 0) inI.boolStack().push(!inI.boolStack().pop());
  }
}
...
class If extends ObjectStackInstruction
{
  ...
  public void Execute(Interpreter inI)
  {
    ...
    if (_stack.size() > 1 && bstack.size() > 0)
    {
      ...
      if (istrue) estack.push(iftrue);
      else estack.push(iffalse);
    }
  }
}
