...
public class ObjectStack extends Stack
{
  ...
  public boolean equals(Object inOther)
  {
    ...
    if (!(inOther instanceof ObjectStack)) return false;
  ...
  {
    if (inOtherSize != _size) return false;

    for (int n = 0; n < _size; n++)
    {
      if (!_stack[n].equals(inOther[n])) return false;
    }
  ...
  {
    if (inValue instanceof Program) inValue = new Program((Program) inValue);

    _stack[_size] = inValue;
    _size++;

    if (_size >= _maxsize) resize(_maxsize + _blocksize);
  }
