...
public class Interpreter implements Serializable
{
   ...
   * <p/>
   * When frames are enabled, each Push subtree is given a fresh set of stacks (a "frame") when it executes. When a frame
   * is pushed, the top value from each stack is passed to the new frame, and likewise when the frame pops, allowing for
   ...
   *
   * @param inInstructionList A program consisting of a list of string instruction names to be placed in the instruction
   *                          set.
   */
      ...
      throws RuntimeException
  {
    ...
    for (int n = 0; n < inInstructionList.size(); n++)
    {
      ...
      else throw new RuntimeException("Instruction list must contain a list of Push instruction names only");
      ...
      if (name.indexOf("registered.") == 0)
      {
        ...
        if (!registeredType.equals("integer") && !registeredType.equals("float") && !registeredType.equals("boolean") && !registeredType.equals(
            "exec") && !registeredType.equals("code") && !registeredType.equals("name") && !registeredType.equals(
            "input") && !registeredType.equals("frame"))
        {
   ...
   */
  public void SetRandomParameters(int minRandomInt,
                                  int maxRandomInt,
   ...
   * <p/>
   * This method assumes that the intepreter is already setup with an active program (typically using \ref Execute).
   *
   * @param inMaxSteps The maximum number of instructions allowed to be executed.
   *
   * @return The number of instructions executed.
  ...
  /**
   * Generates a single random Push atom (instruction name, integer, float, etc) for use in random code generation
   * algorithms.
   *
